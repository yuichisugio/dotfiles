...
install_extensions_from_file() {
    ...
    log_info "現在インストールされている拡張機能から自動生成します..."

    # もしvscode or cursorフォルダが存在しない場合は作成する
    mkdir -p "$(dirname "$extensions_file")"

    # 現在の拡張機能リストを生成
    if generate_current_extensions_list "$app_name" "$extensions_file"; then
        log_success "拡張機能リストを生成しました: $extensions_file"
    else
        log_error "$app_name の拡張機能リスト生成に失敗しました"
        return 1
    fi
fi
